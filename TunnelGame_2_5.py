...
kutloch_DOSAH = 15 #radius kolem kutlocha kde může sebrat kouli
kutloch_SPEED = 4
REVERSE_BONUS = 1
LIVES_START = 5
...
# === Barvy ===
ball_colors = [
    "yellow", "white", "cyan", "magenta", "red", "green", "blue", "orange",
    "purple", "lime", "pink", "turquoise", "gold", "violet", "salmon",
    "lightblue", "deepskyblue", "orchid", "indianred", "greenyellow"
]

tunnel_colors = [
    "gray", "darkgray", "lightgray", "dimgray", "slategray",
    "blue", "navy", "skyblue", "steelblue", "royalblue",
    "green", "darkgreen", "seagreen", "olive", "limegreen",
    "purple", "indigo", "darkviolet", "pink", "mediumorchid",
    "orange", "orangered", "darkorange", "gold", "chocolate"
]
tunnel_color = random.choice(tunnel_colors)
...
# === Zvuk2 ===
def play_explosion2():
    global plop  # Řekni Pythonu, že chceš pracovat s globální proměnnou plop
    def _play():
        winsound.PlaySound("boom2.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)
    threading.Thread(target=_play).start()
        # Výmaz pozůstalých koulí
    balls.clear()  # === DOPLNĚNO: smaž všechny koule po výbuchu
    plop = 0 #jinak je jen lokální a nevymázne se

    # === Zvuk3 ===
def play_OOF3():
    def _play():
        winsound.PlaySound("OOF3.wav", winsound.SND_FILENAME | winsound.SND_ASYNC)
    threading.Thread(target=_play).start()
...
#       print("Ploška nalezena, délka:", rovinka)
        ...
        balls.append([x-5, y+40, color, idx])  # idx slouží jako referenční bod v tunelu
...
# === DOPLNĚNO: Funkce pro vykreslení bitmapy raketky + plamínky ===
def draw_kutloch(x, y):
    pixel_size = 1
    width = 16
    height = len(kutloch_bitmap)

    # Vykresli bitmapu
    for row_idx, row in enumerate(kutloch_bitmap):
        for bit in range(width):
            if (row >> (15 - bit)) & 1:
                px = x + (bit - width // 2) * pixel_size
                py = y + (row_idx - height // 2) * pixel_size
                canvas.create_rectangle(px, py, px + pixel_size, py + pixel_size, fill="red", outline="red")

    # === PLAMÍNKY NA ZÁDI ===
    # Záď je vlevo od kutlocha (za zády)
    flame_colors = ["blue", "#00aaff", "#3399ff", "#44bbff", "#ccffff"]
    for i in range(3):  # počet plamínků
        offset_y = random.randint(-4, 4)
        offset_x = random.randint(-1, 1)
        length = random.randint(3, 6)
        color = random.choice(flame_colors)

        # Plamínek jako čárka dozadu (vlevo)
        canvas.create_line(
            x - 9, y + offset_y,        # začátek: za raketkou
            x - 9 - length, y + offset_y + offset_x,  # konec
            fill=color,
            width=1
        )
...
def collect_balls():
    global plop, tunnel_color, last_plop
    to_remove = []
    for b in balls:
        bx, by, color, _ = b
        if (kutloch_x - kutloch_DOSAH) < bx < (kutloch_x + kutloch_DOSAH) and (kutloch_y - kutloch_DOSAH) < by < (kutloch_y + kutloch_DOSAH):
            to_remove.append(b)
            plop += 1
    for b in to_remove:
        balls.remove(b)
        play_OOF3()
    # Změna barvy každých 10 koulí
    if plop % 10 == 0 and plop != 0 and plop != last_plop:
        tunnel_color = random.choice(tunnel_colors)
        last_plop = plop

def draw_tunnel():
    global tunnel_color, last_plop

    canvas.delete("all")
    canvas.create_rectangle(0, 0, WIDTH, HEIGHT, fill=tunnel_color, outline="")
    ...
    for i in range(len(tunnel) - 1):
        ...
        canvas.create_polygon(
            x1, y1_top, x2, y2_top, x2, y2_bot, x1, y1_bot,
...
#           outline="white"
    ...
    if not kutloch_dead:
        # Nakreslení modré kružnice (jen obrys)
        canvas.create_oval(
            kutloch_x - kutloch_DOSAH, kutloch_y - kutloch_DOSAH,
            kutloch_x + kutloch_DOSAH, kutloch_y + kutloch_DOSAH,
            outline='blue',
            width=1
        )

        draw_kutloch(kutloch_x, kutloch_y)  # === DOPLNĚNO: bitmapová raketka
...
#           check_collision() # neznáme, co to vlastně dělá ..
            collect_balls()
